...
import ml.rabidbeaver.ssl4cups4j.AdditionalKeyStoresSSLSocketFactory;
...
public class CertificateActivity extends Activity {
	...
	private KeyStore loadTrustStore(){
		...
		try {
			fis = openFileInput(SSLScheme.trustfile);
			ts.load(fis, SSLScheme.password.toCharArray());
		}
		catch (Exception e){
			try {
				ts.load(null, SSLScheme.password.toCharArray());
			}
			catch (Exception e1){
				System.out.println(e.toString());
				return null;
		...
		.setPositiveButton("Yes",new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog,int id) {
		        try {
		        	...
		        	trustStore.setKeyEntry(alias, privateKey, SSLScheme.password.toCharArray(), certChain);
		        	FileOutputStream outputStream = openFileOutput(SSLScheme.trustfile, MODE_PRIVATE);
		        	trustStore.store(outputStream, SSLScheme.password.toCharArray());
		        	outputStream.flush();
		        	outputStream.close();
